...
def apply_name(name: str, i):
    return list(map(f(mapping[name]), i))
...
def way_and_back(way: str, back: str) -> list:
    x = domain(way)
    f1 = apply_name(way, x)
    return apply_name(back, f1)


def compose(names: list):
    """applies a series of transforms from index 0
    @names list of strings of the names of the functions to applyName
    @return a dict for use with f()"""
    m = range(27)
    for name in names:
...
class OpsTest(unittest.TestCase):
    ...
    def testNoop(self):
        func = apply_name('noop', range(27))
        self.assertEqual(func, list(range(27)))

    def testIncX(self):
        x = range(27)
        fx = apply_name('incX', x)
        ffx = apply_name('incX', fx)
    ...
    def testRotations4Times(self):
        for name in mapping:
            ...
            for _ in range(4):  # 3 other times makes 4
                x = apply_name(name, x)
            self.assertEqual(x, domain(name), f'{name} applied 4 times should be identity.')

    def testCompose(self):
        transformations = ['rXr', 'rZl', 'rYl']
        test = [0, 1, 2, 4]
        t = compose(transformations)
        rotated = list(map(f(t), test))
        self.assertTrue(all([x is not None for x in rotated]))
